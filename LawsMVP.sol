...
contract Laws {
    ...
    // Constructor: Called when the contract is first created. Sets the owner.
    constructor(address _owner) {
        owner = _owner;
        creationTimestamp = block.timestamp;
    }

    // Modifier: Ensures this function can only be called by the owner and only within the first 90 days.
    modifier onlyOwnerInFirst90Days() {
        require(msg.sender == owner && block.timestamp < creationTimestamp + 90 days, "Only the owner can perform this action within the first 90 days");
        _;
    }

    // Modifier: Ensures this function can only be called by authorized addresses after the first 90 days.
    ...
    function addLaw(string memory _content, address _targetContract, address _whoContract, address _howContract, uint256 _lawId) 
        external 
